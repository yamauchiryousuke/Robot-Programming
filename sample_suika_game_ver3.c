...
struct ball_test next_ball;

void ball(struct ball_test ball){
    glBegin(GL_POLYGON);
    glColor4f(1.0-(ball.size/10),1.0,0.0,0.0);
    /*ballの側*/
    glVertex2d(0,0);
    for(i=0;i<=36;i++){
        ball.bx=ball.r*cos(10*i*PI/180.0);
        ball.by=ball.r*sin(10*i*PI/180.0);
        glVertex2d(ball.bx,ball.by);
    }
    glEnd();
}

void ball1(struct ball_test ball){
    glBegin(GL_POLYGON);
    glColor4f(1.0-(ball.size/10),1.0,0.0,0.0);
    /*ballの側*/
    glVertex2d(0,0);
...
void ball2(struct ball_test ball){
    glBegin(GL_POLYGON);
    glColor4f(1.0-(ball.size/10),1.0,0.0,0.0);
    /*ballの側*/
    glVertex2d(0,0);
...
void display(void)
{
    ...
    glEnable(GL_LINE_STIPPLE);
    glLineStipple(1,0xACF3);
    glTranslatef(next_ball.x,next_ball.y,0);
    glRotatef(next_ball.ang, 0.0, 0.0, 1.0);
    if(paku==0||paku==4){
        ball(next_ball);
    }else if(paku==1||paku==3){
        ball1(next_ball);
        ...
        if(n>0){
            for (int nn=1;nn<=n;nn++){
                ...
                if(disbb<balls[n].r+balls[n-nn].r){
                    ...
                    balls[n].Vx = el_ball*(balls[n].Vx * (balls[n].mb - balls[n-nn].mb) + 2 * balls[n-nn].mb * balls[n-nn].Vx) / (balls[n].mb + balls[n-nn].mb);
                    balls[n].Vy = el_ball*(balls[n].Vy * (balls[n].mb - balls[n-nn].mb) + 2 * balls[n-nn].mb * balls[n-nn].Vy) / (balls[n].mb + balls[n-nn].mb);
                    balls[n-nn].Vx = el_ball*(balls[n-nn].Vx * (balls[n-nn].mb - balls[n].mb) + 2 * balls[n].mb * temp_Vx) / (balls[n].mb + balls[n-nn].mb);
                    balls[n-nn].Vy = el_ball*(balls[n-nn].Vy * (balls[n-nn].mb - balls[n].mb) + 2 * balls[n].mb * temp_Vy) / (balls[n].mb + balls[n-nn].mb);

                    // 二つのボールの中心間の距離を計算
                    float dx = balls[n].x - balls[n-nn].x;
                    float dy = balls[n].y - balls[n-nn].y;
                    float distance = sqrt(dx * dx + dy * dy);

                    // めり込み量を計算
                    float overlap = balls[n].r + balls[n-nn].r - distance;

                    if (overlap > 0) {
                        // めり込んだ分を戻す
                        float overlap_x = overlap * (dx / distance) / 2;
                        float overlap_y = overlap * (dy / distance) / 2;

                        balls[n].x += overlap_x;
                        balls[n].y += overlap_y;
                        balls[n-nn].x -= overlap_x;
                        balls[n-nn].y -= overlap_y;
                    }
                    ...
                    if(balls[n].size==balls[n-nn].size){
                        if(ball_num<50){
                            if(balls[n].size<10){
                                ...
                                balls[n+1].r=balls[n].r+5;  //ball半径//
                                balls[n+1].mb=balls[n].mb-0.1;  //質量(kg)//
                                balls[n+1].mx=0,balls[n+1].my=0,balls[n+1].mang=0;  //変化量//
                                balls[n+1].dt=0.03;  //時間変化量(s)//
                                balls[n+1].Vx=(balls[n].Vx+balls[n-nn].Vx)/2,(balls[n+1].Vy=balls[n].Vy+balls[n-nn].Vy)/2;  //ball初速度(m/s)//
...
void keyboard(unsigned char key, int x, int y)
{
    ...
    switch(key){
        case 'a':
            n = ball_num;
            next_ball.x-=sens;
            if(next_ball.x-next_ball.r<=-xw){
                next_ball.x = xw-next_ball.r-0.1;
            }
            break;
        case 'd':
            n = ball_num;
        ...
        case 'f':
            if(ball_num<50){
                ...
                next_ball.active=1;  //有効1、無効0//
                next_ball.size=0;  //ball大きさ//
                //next_ball.x=-30,next_ball.y=30;  //ball原点//
                next_ball.ang=0;  //ball角度//
                next_ball.r=5;  //ball半径//
                next_ball.mb=10;  //質量(kg)//
                next_ball.mx=0,next_ball.my=0,next_ball.mang=0;  //変化量//
            }
            break;
...
int main(int argc, char *argv[])
{
    next_ball.active=1;  //有効1、無効0//
    next_ball.size=0;  //ball大きさ//
    next_ball.x=0,next_ball.y=0;  //ball原点//
    next_ball.ang=0;  //ball角度//
    next_ball.r=5;  //ball半径//
    next_ball.mb=10;  //質量(kg)//
    next_ball.mx=0,next_ball.my=0,next_ball.mang=0;  //変化量//
    next_ball.Vx=0,next_ball.Vy=0;  //ball初速度(m/s)//
    next_ball.w=0;  //ball初角速度(rad/s)//
